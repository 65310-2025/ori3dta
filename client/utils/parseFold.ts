...
export function importFold(file: string): Fold {
  ...
  if (fold.edges_foldAngle) {
    ...
    fold.edges_foldAngle = fold.edges_foldAngle.map(
  ...
  for (const key in fold) {
    if (
      Object.prototype.hasOwnProperty.call(fold, key) &&
      key.includes(":") &&
      !key.startsWith("Ori3dita:")
    ) {
      delete fold[key];
    }
  }
...
export function exportFold(fold: Fold): string {
  ...
  output.edges_foldAngle = output.edges_foldAngle.map(
    (angle: number) => (angle * 180) / Math.PI,
  );
  //Replace objects like {x:1,y:2} with arrays like [1,2]
  output.vertices_coords = output.vertices_coords.map(
    (vertex: { x: number; y: number }) => [vertex.x, vertex.y],
  );
  output.edges_vertices = output.edges_vertices.map(
    (edge: { vertex1: number; vertex2: number }) => [
      edge.vertex1,
      edge.vertex2,
    ],
  );
