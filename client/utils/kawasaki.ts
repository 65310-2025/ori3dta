...
function getRotationMatrices(fold:Fold, vertexIndex:number, ):null|{theta:number;rho:number;matrix:number[][]}[]{
    /*
    ...
    */
    ...
    const edge_assignments = connectedEdges.map(edgeIndex => fold.edges_assignment[edgeIndex]);
    const edge_foldAngles = connectedEdges.map(edgeIndex => fold.edges_foldAngle[edgeIndex]); //assume radians
    ...
    const thetas = connectedVertices.map(connectedVertexIndex => {
        ...
        const x = connectedVertex.x - vertex.x;
        const y = connectedVertex.y - vertex.y;
...
export function makeKawasakiFoldable(fold:Fold, vertexIndex:number):Array<{theta:number;rho:number}> | null{
    ...
    */
    ...
    const rotationMatricesCopies = Array.from({ length: n }, (_, index) => {
        return rotationMatrices.slice(index).concat(rotationMatrices.slice(0, index));
    });
    ...
    for (const matrix of netMatrices) {
        if (float.eq(matrix[1][0], matrix[0][1])) {
            ...
            candidateCreases.push({theta:theta>=0?theta-Math.PI:theta+Math.PI,rho:-rho}); //add the opposite solution
        } 
    }
