...
export function createEdge(oldfold:Fold,v1:({x:number,y:number}),v2:({x:number,y:number}), foldAngle:number, assignment:string):Fold{
    ...
    */
    ...
    for (let i = 0; i < fold.edges_vertices.length; i++) {
        ...
        if(vertexOnEdge(edgev1,{v1:v1,v2:v2}) || vertexOnEdge(edgev2,{v1:v1,v2:v2})){continue}
    ...
    if(intersections.length === 0){
        ...
        connectVertices(fold,v1index,v2index,foldAngle,assignment,coincidentEdges)
        return fold
    }
...
function splitEdge(fold:Fold,edgeIndex:number,vertexIndex:number):void{
    ...
    */
    ...
    fold.vertices_vertices[oldVertex2Index] = fold.vertices_vertices[oldVertex2Index].filter(v => v!= oldVertex1Index?v:vertexIndex)
    fold.vertices_vertices[oldVertex1Index] = fold.vertices_vertices[oldVertex1Index].filter(v => v!= oldVertex2Index?v:vertexIndex)
    ...
    fold.vertices_edges[oldVertex2Index] = fold.vertices_edges[oldVertex2Index].map(v => v!= edgeIndex?v:fold.edges_vertices.length-1) //replace old edge with new edge, for oldvertex2 only
